...
- **待处理的问题** — 面板顶部的一条状态栏列出会话正在等待的内容（智能体提出的问题，或等待批准的计划），无论窗口是否获得焦点都会显示；点击可跳转到对话记录中的对应行。
- **进行中的会话和真实名称** — 会话选择器会用"● 进行中"标记 Claude Code 进程仍在运行的会话，自动模式会优先选择进行中的会话；通过 `/session-name` 设置的名称会显示出来，取代自动生成的标题。
