...
- **Walkthrough step to turn the task tools back on.** The "Get started" guide gains a step between the hook and the first session that explains the Claude Code 2.1.233 change and runs `Claude Todos: Enable Claude Code task tools` (same confirmation dialog as the panel button). Roadmap R2, step 2.
...
### Changed
- **READMEs explain the Claude Code 2.1.233 task-tools change.** Since Claude Code 2.1.233 (2026-08-14) the task-list tools (`TodoWrite`, `TaskCreate`/`TaskUpdate`/`TaskList`/`TaskGet`) are off by default on Opus 4.8, Sonnet 5, Fable 5, Mythos 5 and newer, so the task list in the panel stays empty unless `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` is set. All five READMEs now carry a callout at the top, a subsection under Install with the `~/.claude/settings.json` `env` snippet that turns the tools back on, and matching notes under Requirements and Known limitations, linking the upstream CHANGELOG entry and the canonical issues [#80015](https://github.com/anthropics/claude-code/issues/80015) and [#86929](https://github.com/anthropics/claude-code/issues/86929). The JetBrains plugin description (`plugin.xml`) carries the same one-line pointer, since the JetBrains Marketplace page does not show the README. Roadmap R2.
...
### Added
- **Simplified and Traditional Chinese (`zh-cn` / `zh-tw`).** The panel, notifications,
  extension metadata and READMEs are now available in Chinese, for both the VS Code
  extension and the JetBrains plugin — same message catalogs, same `package.nls.*` /
  native-toast coverage as the other locales. Locale resolution now also considers
  the script and region subtags: `zh-TW`, `zh-HK` and `zh-MO` resolve to Traditional;
  `zh-CN`, `zh-SG` and `zh` without a region resolve to Simplified. Forceable via the
  `claudeTodos.language` setting.
- The Chinese translations were generated by AI and are awaiting native-speaker
  review — the terminology glossary lives at `docs/i18n/glossary-zh.md`.
