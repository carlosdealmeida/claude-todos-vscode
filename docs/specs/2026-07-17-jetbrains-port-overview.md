...
|----|--------|-----------|------------------------------|
| **SP0** | Desacoplar o core: `SessionCore` compartilhado, `todosWatcher` sem `vscode`, ponte da webview plugável, `src/core/main.ts` (protocolo JSON-lines) | Extensão VS Code **idêntica** (regressão zero) + `core.js` sidecar rodável. Publicável como 0.16.0 sem mudança visível. | **Sim** — refactor interno, coberto por testes |
| **SP1** | Esqueleto Kotlin + JCEF renderizando o painel read-only (árvore + tokens + dashboard), servindo a build Svelte via custom scheme, spawn do sidecar, `jcefBridge`, watch ao vivo | Painel read-only funcional dentro do IntelliJ, instalável localmente | Não (código novo) |
